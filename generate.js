...
async function loadRestaurantLinks() {
    try {
        const linksPath = path.join('data', 'restaurant_links.json');
        if (await fs.pathExists(linksPath)) {
            return await fs.readJson(linksPath);
        }
    } catch (error) {
...
async function generateIndexPage() {
    ...
    const todayDataPath = path.join('data', `lunch_data_${today}.json`);
    let lunchData = {};
    if (await fs.pathExists(todayDataPath)) {
        lunchData = await fs.readJson(todayDataPath);
    }
    
    const restaurantLinks = await loadRestaurantLinks();

    // CEST / Sweden local time (UTC+2)
    const swedenTime = new Date();
    const lastUpdated = swedenTime.toLocaleString('sv-SE', {
...
`;
    ...
    if (Object.keys(lunchData).length > 0) {
        ...
        for (const [restaurantName, restaurantData] of Object.entries(lunchData)) {
            ...
            const restaurantKey = restaurantName.replace('Scraper', '');
            if (restaurantLinks[restaurantKey]) {
                const links = restaurantLinks[restaurantKey];
                if (links.url) {
    ...
    await fs.writeFile(rootIndexPath, html, 'utf-8');
